...
select count(orderNumber) into cantidad from orders where status = estado and orderDate< fechaF and orderDate> fechaI ;
return cantidad;
end//
delimiter ;
...
begin
declare cantidad int default 0;
select count(orderNumber) into cantidad from orders where shippedDate between fecha1 and fecha2;
return cantidad;
end//
delimiter ;
...
select count(produtCode) into cantidad from products where products.productLine = linea;
return cantidad;
end//
delimiter ;
...
if orders.status = cancelado then 
set var -1
else 
set var 0 end if;
return var; 
...
select min(orderDate) into fechamin from orders join customers on customers.customerNumber= orders.customersNumber where customerNumber= NumClie;
return fechamin;
end//
delimiter ;
...
join products on products.productCode= ordendetails.productCode where products.productCode= CodProd;
return UltFec;
end//
delimiter ;
...
select count(customerNumber) into CantidadCl from customers where salesRepEmployeeNumber = NumEmpl;
return CantidadCl;
end//
delimiter ;
...
begin
declare cantidad int default 0;
select quantityOrdered into cantidad from orderdetails where papota= orderNumber and  productCode= papota2;
